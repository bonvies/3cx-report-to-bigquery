...
//
// callLog/abandonedQueueCalls/queueAnsweredCallsByWaitTime/agentLoginHistory/inboundCalls 依各自資料本身
// 的業務時間欄位做 DAY 分區（見 logs/BIGQUERY_SETUP.md），只用 PeriodFrom/PeriodTo 篩選會導致全表掃描，
// 要額外加一個對應欄位的範圍條件才能命中分區；彙總類報表沒有自然時間欄位，PeriodFrom 本身就是分區欄位，
// 不需要額外條件。
