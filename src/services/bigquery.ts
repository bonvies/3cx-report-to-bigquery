...
// userActivity/callDistribution/callCostByExtensionGroup 依各自資料本身的業務時間欄位做 DAY 分區
// （見 logs/BIGQUERY_SETUP.md），只用 PeriodFrom/PeriodTo 篩選會導致全表掃描，要額外加一個對應
// 欄位的範圍條件才能命中分區；彙總類報表沒有自然時間欄位，PeriodFrom 本身就是分區欄位，不需要
// 額外條件。
const PARTITION_FIELD_BY_TABLE: Partial<Record<string, string>> = {
  ...
  callCostByExtensionGroup: 'StartTime',
};
